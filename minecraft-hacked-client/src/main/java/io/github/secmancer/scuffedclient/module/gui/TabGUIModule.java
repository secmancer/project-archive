...
public class TabGUIModule extends Module {
	...
	public TabGUIModule() {
		...
		theme=new StandardTheme(new IColorScheme() {
			...
			public Color getColor (String name) {
				return (Color) Objects.requireNonNull(instance.settings.stream().filter(setting -> setting.configName.equals(name)).findFirst().orElse(null)).getValue();
			}
		},75,9,2,5);
	}
	...
	public static IFixedComponent getComponent (IClient client, IContainer<IFixedComponent> container, Supplier<Animation> animation) {
		return new TabGUI(()->"Modules",client,theme,container,animation,key->key==GLFW.GLFW_KEY_UP,key->key==GLFW.GLFW_KEY_DOWN,key->key==GLFW.GLFW_KEY_ENTER||key==GLFW.GLFW_KEY_RIGHT,key->key==GLFW.GLFW_KEY_LEFT,new Point(5,130),"modules").getWrappedComponent();
	}
